...
def reme_outlier_seek(df,budget_df,X_full,ci):
    ...
    return seek_outlier_df, seek_budget, seek_adj_count, seek_adj_budget_pct, df_result
...
def analysis(df_submit, run_demo, file_path, display_path, main_page, main_page_info):
    ...
    if submitted_form:
        ...
        seek_outlier_df, seek_budget, seek_adj_count, seek_adj_budget_pct, df_result = reme_outlier_seek(df,budget_df,X_full,ci=ci)        
        seek_outlier_df.to_excel('df_A.xlsx')
        df_result.to_excel('df_A_result.xlsx')
        
        # asdf
        
        # Run Scenario B - Reme Pvalue = 7%
        # df_result = copy.deepcopy(df_initial_result)
        seek_budget_df_pv = copy.deepcopy(seek_outlier_df)
