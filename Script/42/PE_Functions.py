...
#     std_70, lower_70, upper_70 = wls_prediction_std(results_predict,alpha=0.30)
    ...
    if 'ETHNICITY' in include_feature:
        budget_df = pd.DataFrame({'EEID':df['EEID'], 'original': df['LOG_SALARY'], 'GENDER': df['GENDER'],'ETHNICITY': df['ETHNICITY'], 'predicted':y_pred,'pred_lower': lower, 'pred_upper': upper, 'pred_stderr': std})
    else:
