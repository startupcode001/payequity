...
def plot_bar(data,col):    
    ...
    scale_min = round(data['COEF_DISPLAY'].min(),2)-0.05    
    fig = px.bar(data, x=col, y="COEF_DISPLAY", color='STAT', template='seaborn', text=[f'${round(100*j)}' for i,j in zip(data[col],data['COEF_DISPLAY'])])
...
#     budget_df.to_excel('check_final_budget.xlsx')
    ...
    df_result_gender['COEF_DISPLAY'] = 1+df_result_gender['COEF']
    ...
    baseline_row = pd.DataFrame({'GENDER':'M', 'COEF':1, 'COEF_DISPLAY':1,'PVALUE':1,'STAT':"No"},index =[0])
    ...
    df_result_eth['COEF_DISPLAY'] = 1+df_result_eth['COEF']
    ...
    baseline_row = pd.DataFrame({'ETHNICITY':baseline_eth.index[0], 'COEF':1, 'COEF_DISPLAY':1,'PVALUE':1,'STAT':"No"},index =[0])
    ...
    return df, df_org, df_validation, message, exclude_col, r2_raw, female_coff_raw, female_pvalue_raw, r2, female_coff, female_pvalue, before_clean_record, after_clean_record,hc_female,X_full,budget_df,exclude_feature, include_feature,df_gender, df_eth, fig_gender_hc,fig_eth_hc, avg_pay, gender_female_pay, gender_nonb_pay, eth_minor_pay,fig_gender_bar, fig_eth_bar
# , fig_gender_bar
...
def analysis(df_submit, run_demo, file_path, display_path, main_page, main_page_info):
    ...
    with st.spinner('Running analysis, Please wait for it...'):
        ...
        df, df_org,  df_validation, message, exclude_col, r2_raw, female_coff_raw, female_pvalue_raw, r2, female_coff, female_pvalue, before_clean_record, after_clean_record,hc_female,X_full,budget_df,exclude_feature, include_feature,df_gender,df_eth,fig_gender_hc,fig_eth_hc,avg_pay, gender_female_pay, gender_nonb_pay, eth_minor_pay,fig_gender_bar, fig_eth_bar = run(df,df_gender_name)     
...
#         test1.plotly_chart(fig_gender_bar, use_container_width=True)
        ...
        metric_net_gap_2.write("<h1 style='text-align: left; vertical-align: bottom;color: Green; font-size: 110%; opacity: 0.7'> 🌐 > -5% </h1>" "For every $100 earned by male employees, how much is earned by female/non-binary employees. For example $90 means that on average women are paid 10% less compared to men. In US, female gender gap is between -5% and +1%.", unsafe_allow_html=True)
