...
def plot_eth_donut(data):
    # explosion    
    # plt.clf()
    n_exp = len(data['Ethnicity'])
    explode = np.repeat(0.02, n_exp).tolist()
    explode = tuple(explode)
    
    data = data.sort_values(by=['Headcount'])
    ...
    plt.pie(data['Headcount'],  labels=data['Ethnicity'],
...
def run(data=None, df_gender_name=None):
    ...
    df,warning_message = clean_req_feature(data = df,feature = "GENDER",valid_feature_list=["F","M","N","O"],warning_message = warning_message, data_type="string")
...
#     budget_df.to_excel('check_final_budget.xlsx')
    ...
    if 'ETHNICITY' in include_feature:
        ...
        df_eth.columns = ['Ethnicity','Headcount']
...
def analysis(df_submit, run_demo, file_path, display_path, main_page, main_page_info):
    ...
    with st.spinner('Running analysis, Please wait for it...'):
        ...
        df, df_org,  df_validation, message, exclude_col, r2_raw, female_coff_raw, female_pvalue_raw, r2, female_coff, female_pvalue, before_clean_record, after_clean_record,hc_female,fig_r2_gender_gap,fig_raw_gender_gap,fig_net_gender_gap,X_full,budget_df,exclude_feature, include_feature,df_gender,df_eth,fig_gender_hc,fig_eth_hc = run(df,df_gender_name)     
        ...
        main_page.markdown("""---""")
        m_col1_but_col1, m_col1_but_col2, m_col1_but_col3 = main_page.columns((1, 1, 1))
        ...
        m_col1_but_col2.markdown("<h1 style='text-align: center; vertical-align: top; font-size: 100%'>Ethnicity</h1>", unsafe_allow_html=True)
        m_col1_but_col2.pyplot(fig_eth_hc)
        
        m_col1_but_col3.markdown("<h1 style='text-align: center; vertical-align: top; font-size: 100%'>Gender</h1>", unsafe_allow_html=True)
        m_col1_but_col3.pyplot(fig_gender_hc)
        
#         with m_col1_but_col3:
#             plost.donut_chart(data=df_gender,theta='HC',color='GENDER_NAME', title = 'Gender %', legend = 'right', use_container_width = True)
